...
<body>
  <main class="modal-overlay" role="dialog" aria-modal="true" aria-labelledby="manageChapterTitle" aria-describedby="manageChapterDesc">
    <section class="modal-box">
      <div class="modal-header">
        <img src="../Images/book-icon.png" alt="Icon" class="modal-icon">
        <div>
          <p style="letter-spacing: 0.12em; text-transform: uppercase; color: #1b678f; font-weight: 700; margin-bottom: 6px;">Chapter Management</p>
          <h2 id="manageChapterTitle" style="margin: 0; color: #0e3a5d;">Manage Chapters for "<?php echo htmlspecialchars($book['title']); ?>"</h2>
        </div>
      </div>
      <p id="manageChapterDesc" style="margin: 10px 0 18px; color: #5f7385; line-height: 1.7;">Add, update, or remove chapters without leaving the modal workflow.</p>
      <form method="POST">
        ...
        <button type="submit" name="add_chapter" class="save-btn">Add Chapter</button>
      </form>
        ...
        <?php while ($chapter = $chapters->fetch_assoc()): ?>
          <div class="form-group" style="border-bottom: 1px solid #ddd; padding: 10px 0;">
            ...
            <div style="float: right;">
              <a href="edit-chapter.php?id=<?php echo $chapter['id']; ?>" class="chapter-btn">Edit</a>
              <a href="delete-chapter.php?id=<?php echo $chapter['id']; ?>&book_id=<?php echo $book_id; ?>" class="cancel-btn" onclick="return confirm('Are you sure?')">Delete</a>
            </div>
          </div>
        <?php endwhile; ?>
      </div>
